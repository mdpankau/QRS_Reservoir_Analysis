...
pdf(file = paste(patient, "delete_Chimera_Env", "Tree", ".pdf", sep = "_"))
...
#+  scale_color_gradientn(breaks = unique(df$Years_Post_Infection),
#                     labels = unique(df$Years_Post_Infection),
#                     colors = c("#e6194B", "#f58231", "#ffe119", 
...
##At this point I have a matrix with DNA sequence names as the columns-
##RNA sequence names as the rows
## I want to match the DNA to the RNA that has the smallest pairwise distance-
## and the smallest days post infection.  Right now, the code matches-
##smallest pairwise distance and alphabetical order of sequence names.
## I can't rename all the sequences by their DPI because they need unique names-
## DPI + sequence counts may get me there, However, the way numbers as strings-
## are processed may be a problem as it will put 11 as earlier than 2.  I can -
##make a key where the days are letters of the alphabet, and convert those-
##letters back to numbers after matching smallest PWD. BUT the rownames can't
## be identical. So I will have to make each time point a letter followed by
## a sequential number.
